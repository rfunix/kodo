...
fn translate_call<'ctx>(
    ...
    // Handle enum discriminant checks inline — ALWAYS, even if in user_functions.
    // These are generated by the type checker and must be inlined because
    // enum values are { i64, i64 } structs, not simple i64.
    if is_enum_discriminant_check(callee) {
        translate_enum_discriminant_check(dest, callee, args, ctx);
        return;
    }

    // Handle enum unwrap operations inline — ALWAYS.
    if is_enum_unwrap(callee) {
        translate_enum_unwrap(dest, callee, args, ctx);
        return;
    }
...
/// Returns true if the callee is an enum discriminant check that should be inlined.
fn is_enum_discriminant_check(callee: &str) -> bool {
    matches!(
        callee,
        "Option_is_some" | "Option_is_none" | "Result_is_ok" | "Result_is_err"
    )
}

/// Returns true if the callee is an enum unwrap that should be inlined.
fn is_enum_unwrap(callee: &str) -> bool {
    matches!(
        callee,
        "kodo_result_unwrap"
            | "kodo_result_unwrap_err"
            | "kodo_result_unwrap_or"
            | "kodo_option_unwrap"
            | "kodo_option_unwrap_or"
            | "Result_unwrap"
            | "Result_unwrap_err"
            | "Result_unwrap_or"
            | "Option_unwrap"
...
/// Inlines an enum discriminant check (e.g., `Option_is_some(val)`).
///
/// Instead of calling a runtime function, extracts the discriminant from the
/// `{ i64, i64 }` struct and compares it to 0 (Some/Ok) or non-zero (None/Err).
fn translate_enum_discriminant_check<'ctx>(
    dest: LocalId,
    callee: &str,
    args: &[Value],
    ctx: &mut ValueCtx<'_, 'ctx>,
    ...
    let Some(val) = translate_value(arg, ctx) else {
        return;
    };

    // Extract discriminant (field 0).
    let disc = if val.is_struct_value() {
        ctx.builder
            .build_extract_value(
                val.into_struct_value(),
                0,
                &unique_name(ctx.name_counter, "disc"),
            )
            .unwrap()
            .into_int_value()
    } else {
    ...
    let pred = match callee {
        "Option_is_some" | "Result_is_ok" => inkwell::IntPredicate::EQ,
        _ => inkwell::IntPredicate::NE,
    };
    let cmp = ctx
        .builder
        .build_int_compare(pred, disc, zero, &unique_name(ctx.name_counter, "edck"))
        .unwrap();
    let result = ctx
        .builder
        .build_int_z_extend(
            cmp,
            ctx.context.i64_type(),
            &unique_name(ctx.name_counter, "edex"),
        )
        .unwrap();

    let result_val: BasicValueEnum<'ctx> = result.into();
    if let Some(alloca) = ctx.local_allocas.get(&dest) {
        ctx.builder.build_store(*alloca, result_val).unwrap();
    }
    ctx.ssa_cache.insert(dest, result_val);
...
/// Inlines an enum unwrap operation (e.g., `kodo_result_unwrap(val)`).
///
/// Extracts the payload (field 1) from the `{ i64, i64 }` enum struct.
/// For `unwrap_or`, uses the second argument as fallback when discriminant != 0.
fn translate_enum_unwrap<'ctx>(
    dest: LocalId,
    callee: &str,
    args: &[Value],
    ctx: &mut ValueCtx<'_, 'ctx>,
    ...
    let payload = if val.is_struct_value() {
        ctx.builder
            .build_extract_value(
                val.into_struct_value(),
                1,
                &unique_name(ctx.name_counter, "unwp"),
            )
            .unwrap()
            .into_int_value()
    } else {
    ...
    // For unwrap_err, the payload is the error (field 1), same extraction.
    // For unwrap_or, use fallback if discriminant != 0.
    let result = if callee.ends_with("unwrap_or") {
        if let Some(fallback_arg) = args.get(1) {
            if let Some(fb_val) = translate_value(fallback_arg, ctx) {
                let fb = fb_val.into_int_value();
                let disc = if val.is_struct_value() {
                    // Already consumed val above, re-translate
                    if let Some(re_val) = translate_value(arg, ctx) {
                        if re_val.is_struct_value() {
                            ctx.builder
                                .build_extract_value(
                                    re_val.into_struct_value(),
                                    0,
                                    &unique_name(ctx.name_counter, "uord"),
                                )
                                .unwrap()
                                .into_int_value()
                        } else {
                ...
                let is_ok = ctx
                    ...
                    .build_int_compare(
                        ...
                        disc,
                        zero,
                        &unique_name(ctx.name_counter, "uorc"),
                    )
                    .unwrap();
                ctx.builder
                    .build_select(is_ok, payload, fb, &unique_name(ctx.name_counter, "uors"))
                    .unwrap()
                    .into_int_value()
            } else {
                payload
