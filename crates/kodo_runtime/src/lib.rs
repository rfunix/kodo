...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Reference counting tests (Phase 39)
    // -----------------------------------------------------------------------

    #[test]
    fn rc_null_handle_is_safe() {
        // Inc/dec on null handle (0) must be a no-op — no crash.
        kodo_rc_inc(0);
        kodo_rc_dec(0);
    }
    ...
    fn rc_alloc_returns_nonzero() {
        ...
        assert_ne!(handle, 0, "kodo_alloc should return a non-zero handle");
        kodo_rc_dec(handle); // free it
    }
    ...
    fn rc_inc_increments_count() {
        ...
        kodo_rc_inc(handle);
        assert_eq!(kodo_rc_count(handle), 3);
        // Dec back to zero to free.
        kodo_rc_dec(handle);
        kodo_rc_dec(handle);
        kodo_rc_dec(handle);
    }
    ...
    #[test]
    fn rc_dec_decrements_and_frees_at_zero() {
        let handle = kodo_alloc(24);
        kodo_rc_inc(handle);
        assert_eq!(kodo_rc_count(handle), 2);
        kodo_rc_dec(handle);
        assert_eq!(kodo_rc_count(handle), 1);
        // Final dec frees the memory — no crash expected.
        kodo_rc_dec(handle);
    }

    #[test]
    fn rc_free_ignores_null() {
        // Calling kodo_free on null must not crash.
        kodo_free(0);
    }
    ...
    #[test]
    fn rc_alloc_write_and_read_data() {
        // Allocate 16 bytes and verify we can write/read user data.
        let handle = kodo_alloc(16);
        assert_ne!(handle, 0);
        let ptr = handle as *mut u8;
        // SAFETY: handle points to 16 valid bytes.
        unsafe {
            std::ptr::write(ptr, 0xAB);
            std::ptr::write(ptr.add(15), 0xCD);
            assert_eq!(std::ptr::read(ptr), 0xAB);
            assert_eq!(std::ptr::read(ptr.add(15)), 0xCD);
        }
        // Verify refcount is still 1 (data writes didn't corrupt header).
        assert_eq!(kodo_rc_count(handle), 1);
        kodo_rc_dec(handle);
    }

    #[test]
    fn rc_multiple_allocs_independent() {
        let h1 = kodo_alloc(8);
        let h2 = kodo_alloc(8);
        assert_ne!(h1, h2);
        kodo_rc_inc(h1);
        assert_eq!(kodo_rc_count(h1), 2);
        assert_eq!(kodo_rc_count(h2), 1);
        kodo_rc_dec(h1);
        kodo_rc_dec(h1);
        kodo_rc_dec(h2);
    }
    ...
    fn rc_alloc_zero_size() {
        // Zero-size allocation should still work (header only).
    ...
    fn rc_inc_string_noop() {
        // String RC is still a no-op — verify no crash.
    ...
    #[test]
    fn rc_inc_dec_roundtrip() {
        // Simulate a variable being shared across three references and
        // then released.
        let handle = kodo_alloc(64);
        kodo_rc_inc(handle); // ref 2
        kodo_rc_inc(handle); // ref 3
        assert_eq!(kodo_rc_count(handle), 3);
        kodo_rc_dec(handle); // ref 2
        kodo_rc_dec(handle); // ref 1
        assert_eq!(kodo_rc_count(handle), 1);
        kodo_rc_dec(handle); // freed
    }

    #[test]
    fn rc_large_alloc() {
        // Allocate 1 MiB and verify refcount is intact.
        let handle = kodo_alloc(1024 * 1024);
        assert_ne!(handle, 0);
        assert_eq!(kodo_rc_count(handle), 1);
        kodo_rc_dec(handle);
    }
