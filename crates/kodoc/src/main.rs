...
enum Command {
    ...
    /// Run tests in a Kōdo source file.
    Test {
...
fn main() {
    ...
    let exit_code = match cli.command {
        ...
        Command::Test {
            ...
            json,
            contracts,
        } => run_test(&file, filter.as_deref(), json, &contracts),
...
/// Runs tests in a Kōdo source file.
///
...
fn run_test(file: &PathBuf, filter: Option<&str>, json: bool, contracts_mode_str: &str) -> i32 {
    let source = match std::fs::read_to_string(file) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("error: could not read file `{}`: {e}", file.display());
