...
fn private_function_call_from_another_module_errors() {
    // Module A has a private function `secret`.
    ...
    let mut checker = TypeChecker::new();
    // Register module A's visibility info before checking B.
...
fn public_function_call_from_another_module_succeeds() {
    // Module A has a public function `greet`.
    ...
    // Module B calls `greet()` — should succeed since it's public.
    let func_main = make_function(
        "main",
        vec![],
        TypeExpr::Named("Int".to_string()),
